...
int WINAPI WinMain(HINSTANCE hi,HINSTANCE hp,LPSTR cmd,int show){
    ...
    RECT r={0,0,FB_W,FB_H};AdjustWindowRect(&r,WS_OVERLAPPEDWINDOW,0);HWND w=CreateWindowExA(0,game_name(),game_name(),WS_OVERLAPPEDWINDOW|WS_VISIBLE,CW_USEDEFAULT,CW_USEDEFAULT,r.right-r.left,r.bottom-r.top,0,0,hi,0);if(!w)return 2;
    ...
    Framebuffer fb={pixels,FB_W,FB_H};uint8_t prev[8]={0};int vk[8]={VK_UP,VK_DOWN,VK_LEFT,VK_RIGHT,'Z','X',VK_RETURN,'H'};game_init(GetTickCount());beta_audio_open();int started=0;char status[256];
    while(running){MSG m;while(PeekMessageA(&m,0,0,0,PM_REMOVE)){TranslateMessage(&m);DispatchMessageA(&m);}Input in={0};for(int i=0;i<8;i++){in.down[i]=(GetAsyncKeyState(vk[i])&0x8000)!=0;in.pressed[i]=in.down[i]&&!prev[i];prev[i]=in.down[i];}if(!started){if(in.pressed[START])started=1;}else if(!in.down[HELP])game_tick(&in);beta_audio_pump();game_draw(&fb);if(!started)beta_title_frame(pixels);StretchDIBits(dc,0,0,FB_W,FB_H,0,0,FB_W,FB_H,pixels,&bi,DIB_RGB_COLORS,SRCCOPY);SetBkMode(dc,TRANSPARENT);SetTextColor(dc,RGB(220,240,255));game_status(status,sizeof status);TextOutA(dc,12,10,status,(int)strlen(status));TextOutA(dc,12,456,game_help(),(int)strlen(game_help()));if(!started||in.down[HELP])beta_onboarding(dc,!started);Sleep(16);}
    beta_audio_close();return 0;
}
#else
#include <stdlib.h>
