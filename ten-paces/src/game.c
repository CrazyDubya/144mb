...
void game_draw(Framebuffer*f){static const uint32_t sky[4]={0x00d79b58,0x00b9794f,0x007a6a68,0x003f4158};clear(f,sky[g.stage]);rect(f,0,390,640,90,0x00865b35);rect(f,270,250,100,140,0x00694029);rect(f,288,300,30,90,0x00291f19);rect(f,50,300,75,90,0x008b4b2c);if(g.stage>=1)rect(f,470,280,85,110,0x005a3928);if(g.stage>=2)line(f,0,210,640,260,0x00342d2a);if(g.stage>=3)rect(f,390,330,55,60,0x002e2521);line(f,0,390,640,390,0x004c3525);cowboy(f,g.px,g.php,0x003e5d73,g.pdive);cowboy(f,g.ex,g.ehp,0x007b2938,g.edive);for(int i=0;i<3;i++){uint32_t c=i==g.slot&&g.phase==0?0x00fff0a5:0x00594535;rect(f,170+i*105,55,90,36,c);for(int k=0;k<g.plan[i];k++)rect(f,177+k*13,63,8,20,0x00211b18);for(int k=0;k<g.enemy[i];k++)rect(f,177+k*13,91,8,6,0x00852f3c);}if(g.phase==1)rect(f,170,105,g.clock*300/180,5,0x00f3d36a);if(g.won)rect(f,170,180,300,110,0x00296b47);if(g.lost)rect(f,170,180,300,110,0x00792b38);}
