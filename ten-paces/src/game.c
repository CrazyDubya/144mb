...
static void act(int who,int action){
    ...
    else if(action==AIM){(*aim)++;if(*aim>2)*aim=2;}
    else if(action==FIRE)shot(who);
    else if(action==INTERACT){if(*cover<3)(*cover)++;else{g.lamp=!g.lamp;g.smoke+=20;}}
    else if(action==SPEAK){if(who){if(g.reputation<0)g.pmorale--;}else g.emorale-=g.temper==NERVOUS?2:1;}
    ...
    if(*x<70)*x=70;if(*x>570)*x=570;
}
static void resolve_bullets(void){
    for(int i=0;i<8;i++){Bullet*b=&g.bullet[i];if(!b->live)continue;b->x+=b->v;
        ...
        if((b->v>0&&b->x>=target)||(b->v<0&&b->x<=target)){
            ...
            if(hit&&*cover>0){(*cover)--;hit=0;g.smoke+=15;}
...
static void finish_encounter(int spared){
    ...
    if(g.spared>=2)g.ally=1;g.stage++;
...
void game_tick(const Input*in){
    ...
    if(g.phase==0){
        ...
        if(g.clock%60==1&&g.beat<3){act(0,g.plan[g.beat]);act(1,g.enemy[g.beat]);g.beat++;}
...
void game_draw(Framebuffer*f){
    ...
    if(g.won)rect(f,150,175,340,125,0x00296b47);if(g.lost)rect(f,150,175,340,125,0x00792b38);
}
void game_status(char*d,size_t n){
    ...
    snprintf(d,n,"SCENE %d/8 %s  ROUND %d  YOU %dHP %d AMMO  OUTLAW %dHP %s %s  REP %+d SPARED %d  PLAN %s/%s/%s%s",
      st+1,place[st],g.round,g.php,g.pammo,g.ehp,temper_name[g.temper],intent,g.reputation,g.spared,
...
Input game_autoplay(int t){
    ...
    if(g.enemy[g.slot]==FIRE||g.enemy[g.slot]==MELEE)want=DIVE;else if(g.emorale<=1)want=SPEAK;else if(ammo<=0)want=RELOAD;else if(!aim)want=AIM;else want=FIRE;
